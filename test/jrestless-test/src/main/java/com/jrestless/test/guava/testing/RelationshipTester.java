...
/*
 * CHECKSTYLE:OFF
 * Copy of https://github.com/google/guava/blob/27c07248101cc4828b22724aaaf56ec330c1f155/guava-testlib/src/com/google/common/testing/RelationshipTester.java
...
final class RelationshipTester<T> {
	...
	private void assertRelated(int groupNumber, int itemNumber, int relatedItemNumber) {
		...
		T item = itemInfo.getValue();
		T related = relatedInfo.getValue();
	...
	private void assertUnrelated(int groupNumber, int itemNumber, int unrelatedGroupNumber, int unrelatedItemNumber) {
		...
		assertWithTemplate("$ITEM must not be $RELATIONSHIP to $OTHER", itemInfo, unrelatedInfo,
				!equivalence.equivalent(itemInfo.getValue(), unrelatedInfo.getValue()));
	}

	private void assertWithTemplate(String template, Item<T> item, Item<T> other, boolean condition) {
		if (!condition) {
			throw new AssertionError(template.replace("$RELATIONSHIP", relationshipName)
