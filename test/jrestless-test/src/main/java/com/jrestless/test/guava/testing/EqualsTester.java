...
/*
 * CHECKSTYLE:OFF
 * Copy of https://github.com/google/guava/blob/27c07248101cc4828b22724aaaf56ec330c1f155/guava-testlib/src/com/google/common/testing/EqualsTester.java
...
public final class EqualsTester {
	...
	private void testItems() {
		for (Object item : Iterables.concat(equalityGroups)) {
			if (item.equals(getNull())) {
				throw new AssertionError(item + " must not be Object#equals to null");
			}
			if (item.equals(NotAnInstance.EQUAL_TO_NOTHING)) {
				throw new AssertionError(item + " must not be Object#equals to an arbitrary object of another class");
			}
			if (!item.equals(item)) {
				throw new AssertionError(item + " must be Object#equals to itself");
			}
			if (item.hashCode() != item.hashCode()) {
				throw new AssertionError("the Object#hashCode of " + item + " must be consistent");
			}
		}
	}

	// just to trick FindBugs which complains about item.equals(null)
